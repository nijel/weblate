...
def prepare_glossary_alternatives(unit):
    ...
    sources = term_records(unit, source=True)
    ...
    common_notes = {unit.note, unit.explanation, unit.source_unit.explanation}
    for side in ("source", "target"):
        for record in term_records(unit, source=side == "source"):
    ...
    targets = sources if "read-only" in unit.all_flags else term_records(unit)
